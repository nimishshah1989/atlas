...
from scripts.forge_runner.config import RunConfig, parse_args
from scripts.forge_runner.stages import RunContext, StageResult
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # ------------------------------------------------------------------ #
    from scripts.forge_runner.deadman import scan_on_startup
...
def _run_dry_run(ctx: RunContext) -> int:
    ...
    from scripts.forge_runner.stages import LocalPickStage
...
def _run_retry(chunk_id: str, ctx: RunContext) -> int:
    ...
    from scripts.forge_runner.state import get_chunk, reset_to_pending
...
def _handle_crash(ctx: RunContext) -> int:
    ...
    try:
        from scripts.forge_runner.logs import write_crash_record
...
def _reset_current_chunk_if_any(ctx: RunContext) -> None:
    ...
    if chunk is None:
        return
    try:
        from scripts.forge_runner.state import reset_to_pending
