...
from scripts.forge_runner.session import run_session  # noqa: E402 – used in LocalImplementStage
...
class LocalPickStage:
    ...
    async def run(self, ctx: RunContext) -> StageResult:
        ...
        from scripts.forge_runner.halt import HaltDecision, evaluate_halt
...
class LocalImplementStage:
    ...
    async def run(self, ctx: RunContext) -> StageResult:
        from scripts.forge_runner import logs, state
        from scripts.forge_runner.session import AuthFailure
...
class LocalVerifyStage:
    ...
    async def run(self, ctx: RunContext) -> StageResult:
        from scripts.forge_runner import logs
...
def _update_runner_state(
    ...
    if last_event and last_event.get("kind") == "tool_use":
        from scripts.forge_runner.secrets import scrub as _scrub  # noqa: PLC0415
